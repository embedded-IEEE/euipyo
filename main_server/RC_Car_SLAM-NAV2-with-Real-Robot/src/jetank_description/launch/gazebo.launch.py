...
def generate_launch_description():
    ...
    # RC car LiDAR 방향 보정 TF (lidar_link_1 -> lidar_link_1_back, yaw 180deg)
    rc_lidar_back_tf = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='lidar_back_tf',
        # x y z yaw pitch roll (yaw = 180deg)
        arguments=['0', '0', '0', '3.14159', '0', '0', 'rc_car/lidar_link_1', 'rc_car/lidar_link_1_back'],
        output='screen',
    ...
    spawn_rc_car = TimerAction(period=5.0, actions=[Node(
        package='ros_gz_sim', executable='create',
        arguments=["-topic", "/rc_car/robot_description", "-name", "rc_car", "-x", "0.0", "-y", "1.0", "-z", "0.32"],
        output='screen'
    )])
    ...
    rc_rf2o_delayed = TimerAction(period=15.0, actions=[rc_rf2o_node])
    ...
    nodes_to_run = [
        ...
        ros_gz_bridge, rc_controllers_delayed, ros_gz_image_bridge,
