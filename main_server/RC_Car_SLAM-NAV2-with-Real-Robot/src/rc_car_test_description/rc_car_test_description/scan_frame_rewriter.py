...
class ScanFrameRewriter(Node):
    def __init__(self):
        ...
        self.declare_parameter('scan_in_topic', '/rc_car/scan_raw')
        self.declare_parameter('scan_out_topic', '/rc_car/scan')
        self.declare_parameter('force_stamp_now', False)
        self.declare_parameter('scan_rate', 0.0)  # Hz, 0.0이면 msg.scan_time 기반
        target_frame = self.get_parameter('target_frame').value
        scan_in_topic = self.get_parameter('scan_in_topic').value
        scan_out_topic = self.get_parameter('scan_out_topic').value
        ...
        self.pub = self.create_publisher(LaserScan, scan_out_topic, pub_qos)
    ...
    def callback(self, msg):
        ...
        if self.force_stamp_now or stamp.nanoseconds == 0:
